...
Before using the APICache you need to initialize the caches. In merb, for example, put this in your <tt>init.rb</tt>:

    APICache.start

By default an in memory hash is used - obviously not a great idea. Thankfully APICache can use any moneta store, so for example if you wanted to use memcache you'd do this:

    require 'moneta/memcache'
    memcache_store = Moneta::Memcache.new(:server => "localhost")

    APICache.start(memcache_store)
