...
All the caching is still handled for you. If you supply a block then the first argument to <tt>APICache.get</tt> is assumed to be a unique key rather than a URL. Throwing <tt>APICache::InvalidResponse</tt> signals to <tt>APICache</tt> that the request was not successful.
